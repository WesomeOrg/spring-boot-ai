...
public class SearchEngineDocumentRetriever implements DocumentRetriever {
    private static final Logger logger = LoggerFactory.getLogger(SearchEngineDocumentRetriever.class);
    ...
    public SearchEngineDocumentRetriever(RestClient.Builder restClientBuilder, int maxResults) {
        ...
        String tavilySearchApiKey = System.getenv(API_KEY);
        Assert.notNull(tavilySearchApiKey, "tavily Search Api Key cannot be null, check Environment variable " + API_KEY);
        this.restClient = restClientBuilder
                .baseUrl("https://api.tavily.com/search")
                .defaultHeader(HttpHeaders.AUTHORIZATION, "Bearer " + tavilySearchApiKey)
                .build();
    ...
    public List<Document> retrieve(Query query) {
        ...
        logger.debug("Searching documents for query: {}, response: {} ", query.text(), response);
